...
bool isPrimitiveRoot(const BigInt& g, const BigInt& p, const vector<BigInt>& primeFactors) {
    BigInt p_minus_1 = p - BigInt(1);
    
    for (const BigInt& k : primeFactors) {
        // Calculate exponent = (p-1)/k
        BigInt exponent = p_minus_1 / k;
        
        // Calculate g^exponent mod p
        BigInt result = BigInt::powerMod(g, exponent, p);
        
        // If result == 1, g is NOT a primitive root
        if (result.isOne()) {
            return false;
        }
    }
...
int main(int argc, char* argv[]) {
    ...
    // Read n (number of prime factors) - also in hex format
    string nHex;
    inFile >> nHex;
    BigInt nBig(nHex);
    int n = 0;
    for (int i = 0; i < nBig.bitLength(); i++) {
        if (nBig.getBit(i)) {
            n |= (1 << i);
        }
    }
    
    // Read the prime factors of (p-1)
    vector<BigInt> primeFactors;
    for (int i = 0; i < n; i++) {
        string factorHex;
        inFile >> factorHex;
        primeFactors.push_back(BigInt(factorHex));
    }
    
    // Read g (the number to test)
    string gHex;
    inFile >> gHex;
    BigInt g(gHex);
    ...
    // Check if g is a primitive root modulo p
    bool result = isPrimitiveRoot(g, p, primeFactors);
